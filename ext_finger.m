% Extracting feature from a fingerprint
%
% Usage:  [ ret ] = ext_finger( img, display_flag );
%
% arguments: img (FingerPrint Image), display_flag
%               
...
% Enhancement 
   ...
   imshow(img), title('Original Image') 
    [ binim, mask, cimg, cimg2, orient_img, orient_img_m ] = f_enhance(img); %redirects to f_enhance function
   figure;
   imshow(binim), title('Binary Image')
...
% Return Minutiae
    ...
    if display_flag == 1
        ...
        elseif minutiae(i, 3) == 2            %Normal ridges
            for k = y1-2: y1 + 2
              for l = x1-2: x1 + 2
